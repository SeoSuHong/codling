	                ...
	                <dd>
						<c:forEach var="nextCorporation" items="${nextCorporations}">
							<c:if test="${nextCorporation.id == nextJobOpening.corporation_id}">
								${nextCorporation.corporateName}
							</c:if>
						</c:forEach>
					</dd>
	                <dd>
	                	<c:forEach var="nextField" items="${nextFields}">
	                		<c:if test="${nextField.jobOpening_no == nextJobOpening.no}">
		                		${nextField.name} | 
			                	<c:forTokens var="career" items="${nextField.career}" delims="/" varStatus="st">
			            			<c:if test="${fn:length(nextField.career) <= 3}">
			              				<c:if test="${career == '신입'}">
				                			<span>${career}</span>&ensp;&ensp;
				                		</c:if>
				                		<c:if test="${career != '신입'}">
